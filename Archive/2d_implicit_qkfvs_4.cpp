...
void u_derivatives(int *edge_nbs, int nbs, double &u1ref, double &u2ref, double *u_dash, int lcell, int rcell, int e)
{
	...
	for (int j = 1; j <= nbs; j++)
	{
		...
		else if (edge[e].status == 'o') //Far field boundary conditions
		{
			...
			u1ref = Mach * cos(theta);
			u2ref = Mach * sin(theta);
		}
		else
		{
...
int construct_neighbours(int edge_nbs[9], int e, int lcell, int rcell, char status)
{
	...
	{
		edge_nbs[++k] = lcell;
		edge_nbs[++k] = edge[(e % 160) + 1].lcell;
		edge_nbs[++k] = (e == 1) ? (edge[160].lcell) : (edge[e - 1].lcell);
	}
	else if (status == 'o')
	{
		edge_nbs[++k] = rcell;
		edge_nbs[++k] = (e == max_edges) ? (edge[e - 159].lcell) : (edge[e + 1].lcell);
		edge_nbs[++k] = ((e % 160) == 1) ? (edge[max_edges].lcell) : (edge[e - 1].lcell);
	}
	return nbs;*/
	else if (status == 'w')
	{
