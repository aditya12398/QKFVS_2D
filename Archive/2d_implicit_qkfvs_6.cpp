...
{
	std::fstream uder;
	uder.open("uder.dat", std::fstream::app);
	...
	if (edge[e].status == 'w') //No slip Boundary conditions
	{
		uref[1] = (cell[lcell].u1);								 //Linear Interpolation
		uref[2] = (cell[lcell].u2);
		//uref[1] = uref[2] = 0;
	}
	else if (edge[e].status == 'o') //Far field boundary conditions
	{
		/*double theta = aoa * pi / 180;
		uref[1] = Mach * cos(theta);
		uref[2] = Mach * sin(theta);*/
	...
	uder << "Edge Number: " << e << " du/dx = " << u_dash[1] << " du/dy = " << u_dash[2] << " dv/dx = " << u_dash[3] << " dv/dy = " << u_dash[4] << "\n";
	uder.close();
} //End of the function
...
*/
/*int get_nbhs(int lcell, int rcell, char status)
{
	int nbs = 6;
	if (status == 'f')
	{
...
{
	//std::fstream nbhsdata("nbd.dat", std::fstream::app);
	int flag, k = 0;
	if (nbs == 6)
	{
		if (status == 'f')
		{
			...
			for (int i = 0; i < cell[lcell].nbhs; i++)
			{
				for (int j = 0; j < cell[rcell].nbhs; j++)
				{
					...
					//if (e == 597)
					//std::cout << "kr = " << k << " connection = " << edge_nbs[k] << std::endl;
				}
			}
			edge_nbs[++k] = lcell;
		}
	...
	/*nbhsdata << "Edge Number: " << e << " k = " << k;
	for (int count = 1; count <= k; count++)
	{
		nbhsdata << " " << edge_nbs[count];
	}
	nbhsdata << std::endl;
	nbhsdata.close();*/
	return k;
...
void viscous_flux(double *G, double nx, double ny, int e, char status)
{
	...
	lcell = edge[e].lcell;
	rcell = edge[e].rcell;
	//nbs = get_nbhs(lcell, rcell, status);
	int nbs = 6;
	if (status == 'f')
	{
		if ((cell[lcell].nbhs == 5) && (cell[rcell].nbhs == 5))
	...
	std::fstream vfluxdata("vflux.dat", std::fstream::app);
	vfluxdata << "Edge Number: " << e << " Fluxes: ";
...
void evaluate_flux()
{
	...
	int lcell, rcell;
	double nx, ny, s;
	double Gp[5], Gn[5], Gd[5];// = {0, 0, 0, 0, 0};
	...
	for (int count = 1; count <= max_cells; count++){
		std::fstream fluxfile("evalflux.dat", std::fstream::app);
		fluxfile << "Cell: " << count << " Flux:";
		for (int i = 1; i <= 4; i++)
			fluxfile << " " << cell[count].flux[i];
		fluxfile << std::endl;
		fluxfile.close();}
...
void KFVS_wall_flux(double *G, double nx, double ny, double rho, double u1, double u2, double pr)
{
	...
	//tx = ny;
	//ty = -nx;
	ut = 0.0; //u1 * tx + u2 * ty; No slip boundary condition
	un = 0.0; //u1 * nx + u2 * ny;
...
void state_update()
{
	...
	//double delt, area;
	max_res = 0.0;
	std::fstream stupdatefile("stup.dat", std::fstream::app);
	double U[5];
	...
	for (int k = 1; k <= max_cells; k++)
	{
		...
		double temp = U[1];
		stupdatefile << "Cell: " << k << " Data:";
		for (int r = 1; r <= 4; r++)
		{
			stupdatefile << " " << cell[k].Upold[r] << " " << cell[k].Upnew[r] << " " << cell[k].Uold[r] << " " << cell[k].Unew[r];
			U[r] = cell[k].Unew[r];
...
void write_tecplot()
{
	...
	tecplotfile << "VARIABLES= \"X\", \"Y\", \"Density\", \"Pressure\", \"x-velocity\", \"y-velocity\", \"Mach Number\"\n";
