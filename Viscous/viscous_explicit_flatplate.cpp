/*
This code is a solver for 2D Navier Stokes equations for compressible domain. This code is specially modified to run a flatplate test case.
...
int main(int arg, char *argv[])
{
    ...
    void state_update();
    void print_output();
    string residue_file = "./Output/residue_viscous_flatplate";
    ofstream outfile(residue_file);
    if (!outfile)
    {
...
void evaluate_flux()
{
    ...
    for (int k = 1; k <= max_edges; k++)
    {
        ...
        else if (status == 'w') //Flux across wall edge
        {
            if (lcell == 0)
            {
                nx = -nx;
...
{
    static int count = 1;
    string title = to_string(count++) + "_flow_viscous_flatplate.dat";
    ...
    tecplotfile << "ZONE I= " << (imax - 1) << " J= " << (jmax - 1) << ", DATAPACKING=BLOCK\n";
...
void print_output()
{
    ofstream outfile("./Output/primitive_vector_viscous_flatplate.dat");
