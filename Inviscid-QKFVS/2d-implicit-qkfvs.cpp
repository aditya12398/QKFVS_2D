...
{
	void flux_G(double *, double, double, double, double, double, double);
	double tx, ty;
	double rho, u1, u2, pr, ut, un;
	...
	flux_G(G, nx, ny, rho, ut, un, pr);
} //End of the function

//Function to find the full - flux across a face
void flux_G(double *G, double nx, double ny, double rho, double ut, double un, double pr)
{
	double e;
	double tx, ty;
	double temp1, temp2;
	...
	*(G + 1) = rho * un; //Mass flux across the face
	...
	*(G + 4) = (pr + rho * e) * un; //Energy Flux across the face
} //End of the flux G
